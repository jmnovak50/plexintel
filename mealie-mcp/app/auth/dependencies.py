...
async def current_rest_principal(
    ...
    access = await request.app.state.identity.verify_token(credential.credentials)
    if access is None:
        raise HTTPException(status_code=401, detail="Invalid bearer token")
    return request.app.state.identity.principal_from_access_token(access)


def current_mcp_principal(identity) -> Principal:
