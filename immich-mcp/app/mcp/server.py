...
def create_mcp_server(
    ...
    server = MCPServer(
        ...
        instructions=(
            ...
            "Preserve an active album scope across follow-ups unless the user changes it. Treat every named "
            "album, person, location, date, media type and count as mandatory. Use search_library for combined "
            "album_id, people_all/people_any/people_none, place, capture dates, media type and OCR; never use "
            "location-only results when a person or album is required. All filters combine with AND. "
            "All filters are required; visual_preference is optional assistant ranking, "
            ...
            "and meaning; inspect shortlisted assets before visual ranking and do not invent scores, duplicate "
            "clusters or claim global best. Reject near-duplicate highlights unless requested. Return fewer "
            ...
            "Do not claim a photo is displayed unless a supported client display path was used. Never call an "
            "asset a photo of a named person or describe that person's activity without returned person evidence "
            "or visual inspection of that exact asset. For 'all', enumerate metadata to complete=true before "
            "claiming completeness; display only a bounded subset and distinguish it from the full match set. "
        ),
        auth=AuthSettings(
